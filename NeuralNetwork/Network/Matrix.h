...
Matrix<T>::Matrix(int height, int width)
    : height(height), width(width), array(std::vector<std::vector<T>>(height, std::vector<T>(width)))
{
}

template <class T>
Matrix<T>::Matrix(std::vector<std::vector<T>> const &array) : height(array.size()), width(array[0].size()), array(array)
{
    assert(array.size() != 0);
}
...
template <class T> int Matrix<T>::getHeight()
{
    return height;
}

template <class T> int Matrix<T>::getWidth()
{
    return width;
}
...
template <class T> void Matrix<T>::put(int h, int w, T const &value)
{
    array[h][w] = value;
}

template <class T> T Matrix<T>::get(int h, int w) const
{
    return array[h][w];
}
...
template <class T> void Matrix<T>::print(std::ostream &flux) const
{
    ...
    std::vector<unsigned int> maxLength(width);
    std::stringstream ss;
...
template <class T> void Matrix<T>::operator+=(Matrix const &m)
{
...
template <class T> void Matrix<T>::operator+=(T const &m)
{
    add(m);
}
